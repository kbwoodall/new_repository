...
- (void)viewDidLoad {
    ...
    myQueue = dispatch_queue_create("com.sm.myqueue", 0);
    ...
    //[NSThread detachNewThreadSelector:@selector(myMethod) toTarget:self withObject:items];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //initialize activity setting for tableView
    
    //NSArray *rows = tableView.indexPathsForSelectedRows;    
    //NSLog(@"value: %d", rows);       
    //for (id object in rows) {
       //NSLog(@"value: %@", tableView);
    //}
   
    UITableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    
    //NSLog(@"value: %@", cell.text);
...
-(void)updatedFeedWithRSS:(NSMutableArray*)items
{
    ...
    dispatch_async(myQueue, ^{
        printf("asyncronous call from updatedFeedWithRSS\n");       
    });
}
...
-(void)myMethod{
    ...
    dispatch_async(myQueue, ^{
        printf("asyncronous call from myMethod\n");
    });
